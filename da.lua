...
function Leaf:CreateWindow(config)
    ...
    function window:CreateTab(props)
        ...
        function tab:Toggle(props)
            ...
            table.insert(self.elements, toggleElement)
        ...
        function tab:CreateDropdown(props)
            ...
            local dropdown = {
                ...
                Type = "Dropdown",
                Name = props.Name,
                GetState = function() return Info.Text end,
        ...
        function tab:CreateColorPicker(props)
            ...
            table.insert(self.elements, colorPickerElement)
        ...
        function tab:Input(props)
            ...
            table.insert(self.elements, inputElement)
    ...
    for _, tab in ipairs(window.allTabs) do
        ...
        for _, element in ipairs(tab.elements) do
            ...
            if element.Type == "ColorPicker" then
                state = {state.R, state.G, state.B}
            end
            config[tab.Name][element.Title or element.Name] = {
                Type = element.Type,
                State = state
            }
        end
...
local function loadConfig(window, config)
    for _, tab in ipairs(window.allTabs) do
        local tabConfig = config[tab.Name]
        if tabConfig then
            for _, element in ipairs(tab.elements) do
                local key = element.Title or element.Name
                local elementConfig = tabConfig[key]
                if elementConfig then
                    if element.Type == elementConfig.Type then
                        local state = elementConfig.State
                        if element.Type == "ColorPicker" then
                            state = Color3.new(state[1], state[2], state[3])
                        end
...
if pcall(function() readfile("LeafConfigsList.json") end) then
    configsList = game:GetService("HttpService"):JSONDecode(readfile("LeafConfigsList.json"))
end
